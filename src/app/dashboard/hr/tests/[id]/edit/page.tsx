...
export default function TestEditPage() {
  ...
  return (
    <div className="animate-fade-in" style={{ padding: '2rem', maxWidth: '1000px', margin: '0 auto' }}>
      <button onClick={() => router.back()} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer', marginBottom: '1rem', fontWeight: 600 }}>
      ...
      <div style={{ background: 'var(--surface)', padding: '2rem', borderRadius: '16px', boxShadow: '0 4px 6px -1px rgba(0,0,0,0.05)', marginBottom: '2rem' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h2 style={{ fontSize: '1.25rem', fontWeight: 700, color: 'var(--text-primary)' }}>Questions Management</h2>
        <button type="button" onClick={() => append({ text: '', type: 'single', options: ['', ''], correctIdx: [0] })} style={{ padding: '0.5rem 1rem', background: 'var(--background)', border: '1px solid var(--border)', borderRadius: '8px', color: 'var(--text-primary)', fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
        ...
        {fields.map((field, index) => (
          <div key={field.id} style={{ background: 'var(--surface)', padding: '1.5rem', borderRadius: '16px', border: '1px solid var(--border)', position: 'relative' }}>
            ...
            <div style={{ display: 'grid', gap: '1rem' }}>
              <div>
                <input {...register(`questions.${index}.text` as const, { required: true })} placeholder="Question text" style={{ width: '100%', padding: '0.75rem', borderRadius: '8px', border: '1px solid var(--border)', outline: 'none', fontSize: '1rem', fontWeight: 600 }} />
              </div>
              
              <div>
                <select {...register(`questions.${index}.type` as const)} style={{ padding: '0.5rem', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--surface)', cursor: 'pointer' }}>
              ...
              <div style={{ padding: '1rem', background: 'var(--background)', borderRadius: '8px' }}>
                <p style={{ fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-secondary)', marginBottom: '0.5rem' }}>Options (Comma separated for demo syntax) - Example: A, B, C</p>
                <input 
                  type="text" 
